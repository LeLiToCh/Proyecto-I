...
public class LogInController extends LogInApplication{
    ...
    private void clickLogin() throws IOException, InterruptedException { // metodo que hace las validaciones, entre ellas: contrasenna y usuario correcto o no de los diferentes usuarios, ademas del cambio de escena.
        ...
        if (username.getText().toString().equals("Elduro") && password.getText().toString().equals("69420")){ // hacemos validacion si el campo de usuario y contrasenna son los de nuestro usuarios (si estan correctos)
            TimeUnit.SECONDS.sleep(1);
            m.changeScene("2ndwindowmplayer.fxml"); // se genera el cambio de escena a la seguna ventana

        }
        if (username.getText().toString().equals("QuitoEsteban69") && password.getText().toString().equals("222")) { // hacemos validacion si el campo de usuario y contrasenna son los de nuestro usuarios (si estan correctos)
            TimeUnit.SECONDS.sleep(1);
            m.changeScene("2ndwindowmplayer.fxml");
        }
        if (username.getText().toString().equals("Emmanuel") && password.getText().toString().equals("310757")) {  // hacemos validacion si el campo de usuario y contrasenna son los de nuestro usuarios (si estan correctos)
            TimeUnit.SECONDS.sleep(1);
            m.changeScene("2ndwindowmplayer.fxml");
        }
